...
double OrientedLineSegment::getWeight( Point2D p ) const { 

	// std::cout << "length " << length() << " distance " << distance(p) << std::endl;

	return pow( pow( length() , P ) / ( A+distance(p) ) , B ); 
}
...
Point2D OrientedLineSegmentPairs::getSourcePosition( Point2D target ) const
{
	...
	for( int i=0 ; i<size() ; i++ )
	{
		...
		totalD += d*weight;
		// std::cout << "totalD " << totalD[0] << " " << totalD[1] << std::endl;
		totalWeight += weight;
	}
