...
namespace Image
{
	...
	void JPEGReadImage(FILE *fp, Image32 &img)
	{
		...
		if (cinfo.output_components != 1 && cinfo.output_components != 3)
			THROW("Wrong number of components: ", cinfo.output_components);
		...
		while (cinfo.output_scanline < cinfo.output_height)
		{
			...
			for (int i = 0; i < row_stride; i++)
			{
				...
				if (cinfo.output_components == 1)
					p.r = buffer[0][i], p.g = buffer[0][i], p.b = buffer[0][i];
				else
				{
					if (i % 3 == 0)
						p.r = buffer[0][i];
				...
				if (cinfo.output_components == 1)
					img(i, cinfo.output_scanline - 1) = p;
				else
					img(i / 3, cinfo.output_scanline - 1) = p;
			}
		}
	...
	void JPEGWriteImage(const Image32 &img, FILE *fp, int quality)
	{
		...
		if (!pixels)
			THROW("Failed to allocate pixels[", row_stride, "]");
